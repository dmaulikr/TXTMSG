...
- (void)setup {
    ...
    self.placeholderLabel.font = [UIFont fontWithName:@"Helvetica-Bold" size:13];
...
-(void)specialCharactersEncountered
{
    ...
    NSString *rawStr = self.textView.text;
    ...
    if ([rawStr length])
	{
		[self addContact:recipient withName:recipient];
	}
}
...
- (BOOL)textView:(UITextView *)textView shouldChangeTextInRange:(NSRange)range replacementText:(NSString *)text;
{
    ...
    { // Return key was pressed
        [self specialCharactersEncountered];
        return NO;
    }
...
*/
-(void)makeContactPickerFirstResponder
{
    [self.textView becomeFirstResponder];
}
@end
